...
import DictData from "views/pages/dict/DictData.vue";
...
const func = {
   ...
   */
  updateDisabled: (...objs) => {
    return updDicTypeDisabled(...objs)
  },
  /**
   * 删除
   * @param ids
   */
  deleteList: (...ids) => {
    return delDictTypeList(...ids)
  }
}

const tEditDict = id => {
  const obj = state.list.find(item => item.id === id)
  state.drawerObj.name = obj.name
  state.drawerObj.type = obj.type
...
<template>
  ...
  >
    <template #header>
      <div>
        <el-tag size="large">
          <p>{{ state.drawerObj.name }} {{ state.drawerObj.type }}</p>
        </el-tag>
      </div>
    </template>
    <DictData
        :dict-type="state.drawerObj.type"
    />
  </el-drawer>
  ...
  >
    ...
    >
      ...
      <el-form-item :label="state.dict['name']" prop="name">
        <el-input ref="dialogFormInput1Ref" v-model="state.dialogForm['name']" :placeholder="state.dict['name']"/>
      </el-form-item>
      <el-form-item :label="state.dict['type']" prop="type">
        <el-input v-model="state.dialogForm['type']" :placeholder="state.dict['type']"
                  :disabled="state.type.value!=='ins'"/>
      </el-form-item>
  ...
  >
    ...
    <el-table-column prop="type" :label="state.dict['type']" width="120">
      <template #default="{row}">
        <span>{{ row.type }}</span>
        <el-button v-no-more-click v-dangerous-operation="{resolve:()=>tEditDict(row.id)}" link type="primary"
                   size="small">编辑
        </el-button>
      </template>
    </el-table-column>
    ...
    <el-table-column :label="state.dict['disabled']" width="80">
      <template #default="{row}">
        ...
        />
        <template v-else>{{ row['disabled'] }}</template>
      </template>
    </el-table-column>
